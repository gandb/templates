...
async function updateImages() {
  ...
  for (let actor of actors) {
    ...
    if(!oldImg)
    {
      console.log(`O ator ${actor.name} não possui imagem.`); 
    }

    if(!oldTokenImg)
    {
      console.log(`O ator ${actor.name} não possui token.`); 
    }

    if(oldImg.indexOf("http") !== -1)
    {
      console.log(`A imagem ${actor.name} do ator é uma URL e vai ser ignorada.`); 
      oldImg=null;
    }
    ...
    if(oldImg.indexOf("http") !== -1)
    {
      console.log(`A imagem ${actor.name} do ator é uma URL`); 
      oldImg=null;
    }
    ...
    if(exist(oldTokenImg)) {
      console.log(`O token ${oldTokenImg} do ator existe.`);
      continue;
    }
    else{
      console.log(`O token ${oldImg} do ator não existe.`);
    }
