...
test("Parse date with parseYYYYMMDDHHMMSSMMM", () => {
    ...
    expect(() => { DatesUtil.parseYYYYMMDDHHMMSSMMM(""); }).
        toThrow("Date format YYYYMMDDHHMMSSMMM incorrect!");
...
test("Add hours ", () => {
    const date: Date = DatesUtil.parse(1990, 5, 1, 2, 3, 4, 5);

    const expectedMajor: Date = DatesUtil.parse(1990, 5, 1, 3, 3, 4, 5);

    const expectedMinor: Date = DatesUtil.parse(1990, 5, 1, 1, 3, 4, 5);

    const expectedAnotherDay: Date = DatesUtil.parse(1990, 5, 2, 2, 3, 4, 5);
