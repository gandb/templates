...
export const handler = (event:any, context:Context, callback:any) => {
  ...
  const apiKey = process.env.CHURCH_DB_PORT;
    ...
    if (1>=0 ) {
        ...
        callback(null, {
          ...
          body:"Pong payload v2.0:" + JSON.stringify(event.body) + ",url:," + url +
           ",multiValueQueryString->idade:" +  JSON.stringify(event.multiValueQueryStringParameters.idade ) +  
           "var-env:"  + apiKey + ",queryString->idade:" +  JSON.stringify(event.queryStringParameters.idade )+ ",method: " + event.httpMethod
